...
Работа с городом начинается в `/admin/cities/{city_slug}`. Вкладка хранится в URL и
ведёт в городские выборки мест, качества, проверки, фото, импорта, обогащения,
маршрутов, публикации и истории.

Сводные разделы:

- `/admin/quality` — качество городов;
- `/admin/system-health` — сервисы, очереди и инциденты;
- `/admin/analytics` — продуктовые и операционные агрегаты;
- `/admin/system-logs` — поиск по request/place/route/city correlation.

Опасные действия подтверждаются через диалог. После успеха UI повторно загружает
backend state; недоступное повторное действие блокируется.
