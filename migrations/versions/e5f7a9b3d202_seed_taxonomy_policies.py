...
def upgrade()->None:
 ...
 op.execute("UPDATE categories SET route_policy='useful_only', is_route_eligible=FALSE WHERE code IN ('pharmacy','bank','atm','parking','transport','bus_stop','toilets','information')")
 op.execute("UPDATE categories SET route_policy='forbidden', is_route_eligible=FALSE WHERE code IN ('hospital','police','shelter')")
 op.execute("UPDATE categories SET route_policy='manual_review', is_route_eligible=FALSE WHERE code IN ('service','unknown')")
 for source,key,value,target,priority,confidence in MAPPINGS:
  op.execute(f"INSERT INTO taxonomy_mappings (source,source_key,source_value,target_category_id,priority,confidence,active,conditions,conditions_hash,fallback,comment,created_by,created_at,updated_at) SELECT '{source}','{key}','{value}',id,{priority},{confidence},TRUE,'{{}}','seed-{source}-{key}-{value}',FALSE,'Базовое сопоставление City GO','migration',CURRENT_TIMESTAMP,CURRENT_TIMESTAMP FROM categories WHERE code='{target}' AND NOT EXISTS (SELECT 1 FROM taxonomy_mappings WHERE source='{source}' AND source_key='{key}' AND source_value='{value}')")

def downgrade()->None:op.execute("DELETE FROM taxonomy_mappings WHERE created_by='migration' AND comment='Базовое сопоставление City GO'")
