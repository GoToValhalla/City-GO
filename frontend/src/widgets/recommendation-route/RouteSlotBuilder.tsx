...
const SLOT_TYPES = ['cafe', 'park', 'museum', 'food', 'viewpoint', 'landmark', 'history', 'culture', 'walk']
...
export const RouteSlotBuilder = ({ citySlug, form, loading, onChange }: Props) => {
  ...
  const patchSlot = (slotId: string, patch: Partial<RouteBuilderSlot>) => {
    onChange({ buildMode: 'constructor', routeSlots: slots.map((slot) => (slot.slot_id === slotId ? { ...slot, ...patch } : slot)) })
  }
  ...
  const loadOptions = async () => {
    const payload = buildRecommendationRouteRequest({ ...form, routeSlots: slots, buildMode: 'constructor' }, citySlug)
    if (!payload.ok) {
  ...
  return (
    <section className="route-control-block route-slot-builder" aria-label="Ручной сценарий прогулки">
      <div className="route-control-title"><ListPlus size={18} /><span>Сценарий прогулки из слотов</span></div>
      <p className="route-start-note">Собери порядок прогулки сам: например кофе → парк → музей → еда → видовая точка.</p>
      <div className="route-chip-row">
        <button type="button" className={slots.length ? 'is-selected' : ''} onClick={addSlot} disabled={loading || slots.length >= 8}>Добавить слот</button>
          ...
          {slots.map((slot, index) => {
            ...
            return (
              <article className="route-slot-card" key={slotId}>
                <div className="route-slot-head"><strong>{index + 1}. {categoryLabel(slot.category || slot.type || '')}</strong><button type="button" onClick={() => removeSlot(slotId)} disabled={loading}><Trash2 size={14} /> Удалить</button></div>
                <div className="route-field-grid">
                  <select value={slot.category || slot.type || ''} onChange={(event) => patchSlot(slotId, { category: event.target.value, type: event.target.value, selected_place_id: null })}>{SLOT_TYPES.map((type) => <option value={type} key={type}>{categoryLabel(type)}</option>)}</select>
                  <select value={slot.duration ?? ''} onChange={(event) => patchSlot(slotId, { duration: event.target.value ? Number(event.target.value) : null })}><option value="">Обычная длительность</option><option value="15">15 минут</option><option value="30">30 минут</option><option value="45">45 минут</option><option value="60">60 минут</option></select>
                </div>
                ...
                {options.length ? <div className="route-slot-options">{options.map((option) => <button type="button" className={slot.selected_place_id === option.place_id ? 'is-selected' : ''} key={option.place_id} onClick={() => patchSlot(slotId, { selected_place_id: option.place_id })}><span>{option.title}</span><small>{option.walk_minutes ? `${option.walk_minutes} мин пешком` : option.address || categoryLabel(option.category)}</small></button>)}<button type="button" onClick={() => patchSlot(slotId, { selected_place_id: null })}><RefreshCw size={14} /> Заменить выбор</button></div> : null}
              </article>
            )
          })}
        </div>
