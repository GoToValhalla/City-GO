...
import { interestOptions } from '../recommendation-route/chipOptions'

type Props = { citySlug: string }

const fallbackCategories = interestOptions.map((item) => ({ code: item.value, name: item.label }))

export const RandomRouteDraftEditor = ({ citySlug }: Props) => {
  ...
  const build = async () => {
    await run(async () => {
      setDraft(await createRandomDraft({
        ...
        category_mode: selected.length ? 'balanced' : 'none',
      }))
      setSearch([])
    })
  }
  ...
  return (
    <section className="route-config-tile route-draft-editor">
      ...
      <div className="route-draft-controls">
        ...
        <div className="route-chip-row">{categories.slice(0, 10).map((item) => (
