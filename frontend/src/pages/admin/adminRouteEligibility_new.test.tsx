...
const cities = { items: [{ id: 1, slug: 'kemerovo', name: 'Кемерово', country: 'RU', region: null }], total: 1, limit: 100, offset: 0 }
const eligibility = { items: [{ place_id: 10, title: 'Краеведческий музей', slug: 'museum', category: 'museum', eligible: true, quality_score: 90, quality_bucket: 'high', reasons: [], primary_reason: 'selected', city_slug: 'kemerovo', placeholder_name: false, high_quality_route_candidate: true }, { place_id: 11, title: 'Культурное место OSM 15446625', slug: 'osm-culture', category: 'culture', eligible: false, quality_score: 70, quality_bucket: 'medium', reasons: ['placeholder_title'], primary_reason: 'placeholder_title', city_slug: 'kemerovo', placeholder_name: true, high_quality_route_candidate: false }], total: 125, limit: 50, offset: 0 }
const diagnostics = { city_slug: 'kemerovo', city_name: 'Кемерово', places_total: 125, eligible_places: 80, published_places: 100, blockers_count_by_reason: { placeholder_title: 12, no_photo: 30 }, near_ready_places: [], sample_blocked_places: [] }
...
const renderPage = () => render(<MemoryRouter initialEntries={['/admin/routes/eligibility?city_slug=kemerovo']}><Routes><Route path="/admin/routes/eligibility" element={<AdminRouteEligibilityPage />} /></Routes></MemoryRouter>)
const fetchUrl = (input: unknown) => input instanceof Request ? input.url : String(input)
const fetchUrls = () => (globalThis.fetch as unknown as { mock: { calls: unknown[][] } }).mock.calls.map((call) => fetchUrl(call[0]))
...
describe('AdminRouteEligibilityPage quality gates', () => {
  beforeEach(() => {
    ...
    vi.spyOn(globalThis, 'fetch').mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url.includes('/admin/routes/eligibility?')) return failEligibilityList ? Promise.resolve(new Response(JSON.stringify({ detail: 'gateway failed' }), { status: 502, headers: { 'x-request-id': 'elig-502' } })) : jsonResponse(eligibility)
      if (url.includes('/admin/places/bulk/apply')) return failBulkApply ? jsonResponse({ detail: 'bulk failed' }, 500) : jsonResponse({ affected_count: 1, warnings: [] })
      return jsonResponse({})
    })
  })

  afterEach(() => { clearAdminSession(); vi.restoreAllMocks(); vi.unstubAllGlobals(); vi.unstubAllEnvs(); cleanup() })
  ...
  it('high-quality preset sends quality gate query params', async () => {
    ...
    await waitFor(() => expect(fetchUrls().some((url) => url.includes('readiness=high_quality') && url.includes('quality=high') && url.includes('min_quality_score=75'))).toBe(true))
  })
