...
export const AdminImportJobsPage = () => {
  ...
  const loadDetail = useCallback((cityId: number) => {
    setError(null)
    adminGet<AdminImportJob>(`/admin/import-jobs/${cityId}`, { cache: false }).then((detail) => { setSelected(detail); scrollToDetail() }).catch((e: Error) => setError(`Детали не открылись: ${e.message}`))
  }, [])
