...
import { adminPost, adminPostLong } from './adminApi'
import { adminDownload } from './adminDownload'
import { STATUS_LABEL, batchFilePath } from './adminEnrichmentHelpers'
import type { EnrichmentAIResult, EnrichmentBatchMeta, ImportApplyResult, ImportPreviewResult } from './adminEnrichmentTypes'
...
export const AdminEnrichmentBatchTable = ({ batches, onRefresh }: Props) => {
  ...
  const [aiResult, setAiResult] = useState<EnrichmentAIResult | null>(null)
  ...
  const runAiEnrich = async (batchId: string) => {
    const ok = window.confirm(`AI-обогатить до ${aiLimit} мест в пакете? Будут подготовлены описания, но в базу они попадут только после проверки и применения.`)
    if (!ok) return
    setBusy(`ai:${batchId}`)
    setError(null)
    setAiResult(null)
    try {
      const result = await adminPostLong<EnrichmentAIResult>(`/admin/place-enrichment/batches/${batchId}/ai-enrich`, {
        limit: aiLimit,
        force: false,
        fields: ['description'],
      })
      setAiResult(result)
      onRefresh()
      setBatchView(batchId, 'ai')
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Ошибка AI-обогащения')
    } finally {
      setBusy(null)
    }
  }
  ...
  return <div>
    <section className="admin-help-panel">
      <div className="admin-help-title">AI-обогащение без терминала</div>
      <p className="admin-page-subtitle">Сформируйте экспорт, нажмите «AI-обогатить», затем проверьте предпросмотр и примените изменения. В базу ничего не пишется до кнопки «Применить».</p>
      <div className="admin-actions-cell" style={{ marginTop: 10 }}>
        <label className="admin-field-inline">Лимит за запуск <select value={aiLimit} onChange={(e) => setAiLimit(Number(e.target.value))}><option value={1}>1 место</option><option value={5}>5 мест</option><option value={10}>10 мест</option><option value={20}>20 мест</option></select></label>
      </div>
    </section>
    ...
    <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Пакет</th><th>Статус</th><th>Город</th><th>Мест</th><th>Поля</th><th>Артефакты</th><th>Действия</th></tr></thead><tbody>{batches.map((b) => {
      ...
      const canAiEnrich = b.status === 'exported'
    ...
    {aiResult && <div className="admin-detail-panel"><h3>AI-результат</h3><p>Модель: <strong>{aiResult.model}</strong></p><p>Обработано: <strong>{aiResult.rows_processed}</strong> · подготовлено изменений: <strong>{aiResult.rows_updated}</strong></p>{aiResult.errors.length > 0 && <div className="admin-state admin-state-error">{aiResult.errors.join('; ')}</div>}<div className="admin-actions-cell"><button className="admin-btn admin-btn-primary admin-btn-sm" onClick={() => void runPreview(aiResult.batch_id)}>Проверить изменения</button><button type="button" className="admin-btn admin-btn-sm" onClick={() => void downloadFile(aiResult.batch_id, 'enriched.csv')}>Скачать enriched.csv</button></div></div>}
