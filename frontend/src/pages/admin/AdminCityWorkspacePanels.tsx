  ...
  return (
    <>
      <div className="admin-metrics-grid admin-metrics-small">
        ...
        <div className="admin-metric-card"><div className="admin-metric-value">{data.readiness.readiness_score}%</div><div className="admin-metric-label">Readiness · {data.readiness.quality_status}</div></div>
        ...
        <div className="admin-metric-card"><div className="admin-metric-value">{data.city.pending_photos ?? 0}</div><div className="admin-metric-label">Фото на ревью</div></div>
      </div>

      <div className="admin-detail-panel">
        <h3>Import / Enrichment</h3>
        <p>Job #{job.job_id ?? '—'} · {job.status} · {job.current_step_label ?? job.current_step}</p>
        <p>Found/saved: {job.places_found ?? 0}/{job.places_saved ?? 0} · processed: {job.processed_items ?? 0}/{job.total_items ?? 0} · retry: {job.retry_count ?? 0}</p>
        <p>Updated: {job.updated_at ?? '—'}</p>
        {job.last_error && <p className="admin-error-text">{job.last_error}</p>}
        <p>{job.next_step}</p>
        {job.step_details && <p className="admin-muted">Warnings/details: {JSON.stringify(job.step_details)}</p>}
        <div className="admin-actions-cell">
          ...
          <button type="button" className="admin-btn admin-btn-sm" disabled={busy === 'enrich'} onClick={() => onImportAction('enrich')}>{actionLabel.enrich}</button>
        </div>
      </div>

      <div className="admin-detail-panel">
        <h3>Публикация и качество</h3>
        <p>Coverage: адресов нет {coverage?.places_without_address ?? 0}, фото нет {coverage?.places_without_photo ?? 0}</p>
        <div className="admin-actions-cell">
          ...
          <Link className="admin-btn admin-btn-sm" to={`/admin/routes/data-quality?city=${data.city.slug}`}>Data quality</Link>
          <Link className="admin-btn admin-btn-sm" to={`/admin/coverage?city=${data.city.slug}`}>Coverage</Link>
        </div>
      </div>
    </>
  )
