...
export const AdminPipelineEnrichmentPanel = ({ form }: Props) => {
  ...
  const runPipeline = async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : 'Ошибка автоматического обогащения')
    } finally {
      setRunning(false)
    }
  }
  ...
  return (
    <section className="admin-detail-panel" style={{ marginBottom: 16 }}>
      <h3>Автоматическое обогащение</h3>
      <p>Собирает исходные наблюдения, доверие по полям, фото-кандидаты, решения публикации и очередь проверки.</p>
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginTop: 12 }}>
        ...
        <button className="admin-btn admin-btn-primary" disabled={running || !form.citySlug} onClick={() => void runPipeline()}>
          {running ? 'Обогащение выполняется...' : 'Запустить обогащение'}
        </button>
      </div>
...
const PipelineSummary = ({ result, steps }: { result: PipelineRunResponse; steps: ImportJobStep[] }) => (
  <div style={{ marginTop: 14 }}>
    <div style={{ color: '#34c759' }}>Задача #{result.job_id} · {statusLabel(result.status)}</div>
