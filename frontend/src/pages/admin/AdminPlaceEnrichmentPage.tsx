...
export const AdminPlaceEnrichmentPage = () => {
  const [params] = useSearchParams(); const form = useEnrichmentForm(); const [batches, setBatches] = useState<EnrichmentBatchMeta[]>([]); const [batchesLoading, setBatchesLoading] = useState(true); const [batchesError, setBatchesError] = useState<string | null>(null)
  const loadBatches = useCallback(() => { setBatchesLoading(true); setBatchesError(null); return adminGet<{ items: EnrichmentBatchMeta[]; total: number }>('/admin/place-enrichment/batches').then((r) => setBatches(r.items)).catch((e: Error) => setBatchesError(e.message)).finally(() => setBatchesLoading(false)) }, [])
  useEffect(() => { void Promise.resolve().then(loadBatches) }, [loadBatches])
  useEffect(() => { const city = params.get('city'); if (city && city !== form.citySlug) form.setCitySlug(city) }, [params, form.citySlug, form.setCitySlug])
