...
from services.anonymous_ownership import hash_ownership_token
...
def _payload(city_slug: str, **extra):
    body = {
        "city_slug": city_slug,
        "budget_minutes": 120,
...
def _create(client, city_slug: str):
    response = client.post("/routes/random", json=_payload(city_slug))
...
def test_draft_owner_can_read_and_mutate_new(client, city_factory, published_place_factory):
    ...
    created, token = _create(client, city.slug)
    draft_id = created["draft_id"]

    read = client.get(f"/routes/drafts/{draft_id}", headers=_headers(token))
...
def test_draft_missing_wrong_credentials_rejected_new(client, city_factory, published_place_factory):
    ...
    assert client.get(f"/routes/drafts/{draft_id}").status_code == 404
    query_leak = client.get(f"/routes/drafts/{draft_id}", params={"session_token": token})
    assert query_leak.status_code == 404

    wrong = client.get(f"/routes/drafts/{draft_id}", headers=_headers("other-session-token-bbbxx"))
    assert wrong.status_code == 404
    assert wrong.json()["detail"]["code"] == "DRAFT_NOT_FOUND"
    assert "other-session-token-bbbxx" not in wrong.text
    assert "points" not in wrong.json().get("detail", {})

    mutate = client.post(
        f"/routes/drafts/{draft_id}/add-point",
        headers=_headers("other-session-token-bbbxx"),
        json={"place_id": created["points"][0]["place_id"], "version": created["version"]},
    )
    assert mutate.status_code == 404
    assert "other-session-token-bbbxx" not in mutate.text
...
def test_draft_expired_and_inactive_rejected_new(client, db_session, city_factory, published_place_factory):
    ...
    expired = client.get(f"/routes/drafts/{draft.id}", headers=_headers(token))
    assert expired.status_code == 404

    draft.expires_at = datetime.utcnow() + timedelta(hours=1)
    draft.status = "closed"
    db_session.commit()
    closed = client.get(f"/routes/drafts/{draft.id}", headers=_headers(token))
...
def test_get_is_idempotent_and_does_not_persist_stale_reconcile_new(
    ...
    first = client.get(f"/routes/drafts/{draft.id}", headers=_headers(token))
    second = client.get(f"/routes/drafts/{draft.id}", headers=_headers(token))
...
def test_exception_after_reconcile_rolls_back_new(
    ...
    loaded = get_public_draft_or_error(
        db_session, draft.id, session_token=token, for_update=True
    )
    with patch(
...
def test_draft_city_unpublished_hides_draft_new(client, db_session, city_factory, published_place_factory):
    ...
    hidden = client.get(f"/routes/drafts/{created['draft_id']}", headers=_headers(token))
