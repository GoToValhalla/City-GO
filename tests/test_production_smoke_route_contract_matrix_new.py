...
def _result(payload: dict[str, object], status: int = 200):
    return validate_route_response(json.dumps(payload, ensure_ascii=False), status)
...
def test_production_smoke_rejects_placeholder_and_service_titles_new(title: str) -> None:
...
@pytest.mark.parametrize("category", sorted(FORBIDDEN_ROUTE_JUNK))
def test_production_smoke_rejects_forbidden_route_categories_new(category: str) -> None:
    payload = _payload(points=[{"title": "Museum", "category": "museum"}, {"title": "Bad point", "category": category}])
...
def test_production_smoke_rejects_large_budget_overflow_without_honest_reason_new() -> None:
    payload = _payload(total_places=3, total_estimated_minutes=300, time_budget_minutes=120)
