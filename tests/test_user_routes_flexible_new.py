...
def test_user_route_build_structured_returns_slot_options_new(client, city_factory, place_factory):
    ...
    payload = {
        **_route_payload("slot-city"),
        "build_mode": "constructor",
        "slots": [{"slot_id": "s1", "category": "cafe", "preferred_place_id": None}],
    }
...
def test_user_route_update_reorders_existing_places_new(client, city_factory, place_factory):
    ...
    response = client.post(
        ...
        json={"current_route": _state([first.id, second.id], "update-city"), "ordered_place_ids": [str(second.id), str(first.id)]},
    )

    assert response.status_code == 200
    points = response.json()["points"]
    assert points[0]["place_id"] == str(second.id)


def test_user_route_replace_place_updates_route_new(client, city_factory, place_factory):
    ...
    response = client.post(
        ...
        json={"current_route": _state([old.id], "replace-city"), "old_place_id": str(old.id), "new_place_id": str(new.id)},
    )
...
def test_user_route_add_place_appends_place_new(client, city_factory, place_factory):
    ...
    response = client.post(
        ...
        json={"current_route": _state([first.id], "add-city"), "place_id": str(added.id)},
    )
...
def test_user_route_alternatives_from_state_new(client, city_factory, place_factory):
    ...
    response = client.post(
        ...
        json=_state([current.id], "alt-city"),
    )
