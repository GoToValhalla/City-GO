...
def test_emergency_hide_hides_place_and_writes_audit_and_transition(
    client: TestClient,
    db_session,
    ...
    assert place.publication_reason_code == "admin_hide"
    ...
    assert transition.reason_code == "admin_hide"
    assert transition.source == "admin_emergency_hide"
    assert transition.correlation_id == "hide-key-2"


def test_emergency_hide_is_idempotent_new(client: TestClient, db_session, published_place_factory) -> None:
    place = published_place_factory(category="museum")
    payload = {
        "reason": "Срочное скрытие из-за подтвержденной ошибки данных",
        "idempotency_key": "hide-key-3",
    }
    ...
    assert db_session.query(PlacePublicationTransition).filter_by(place_id=place.id).count() == 1


def test_emergency_hide_removes_place_from_public_catalog_new(
    client: TestClient,
    city_factory,
    published_place_factory,
) -> None:
