...
def update_admin_place_fields(db:Session,place_id:int,fields:dict[str,object],*,actor:str)->Place|None:
 ...
 if category_changed:
  code=str(updates.get("canonical_category") or updates.get("category") or "").strip().lower();category=db.query(Category).filter(Category.code==code,Category.is_active.is_(True)).first()
  if category is None:raise ValueError("Выберите существующую активную категорию")
 ...
 write_admin_audit_log(db,actor=actor,action="update_place_admin",entity_type="place",entity_id=place.id,old_value=old,new_value=updates,reason=str(reason) if reason else None)
 record_event(db,event_type="place_updated",place_id=place.id,commit=False);db.commit();db.refresh(place)
