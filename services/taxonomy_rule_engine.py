...
def classify_place(
    ...
    *,
    ...
    """Классифицирует место по строгому приоритету и никогда не скрывает неопределённость."""
    del city_context
    ...
    if manual_category_id is not None:
        ...
        if category:
            return ClassificationResult(category.id, category.code, 1.0, None, "Категория выбрана администратором.", "manual")

    candidates = _mapping_candidates(db, source=source, tags=tags)
    if candidates:
        best_score, best = candidates[0]
        alternatives = [
            {"category_id": mapping.target_category_id, "confidence": round(score, 3), "mapping_id": mapping.id}
            for score, mapping in candidates[1:4]
        ]
        ambiguous = bool(alternatives and abs(best_score - float(alternatives[0]["confidence"])) < 0.05)
        category = db.query(Category).filter(Category.id == best.target_category_id).first()
        warnings = ["Несколько правил дали близкий результат."] if ambiguous else []
        confidence = min(1.0, best_score)
        decision = _decision(confidence, ambiguous=ambiguous)
        return ClassificationResult(
            best.target_category_id,
            category.code if category else None,
            confidence,
            best.id,
            f"Совпало правило {best.source}:{best.source_key}={best.source_value}.",
            decision,
            warnings,
            alternatives,
        )

    legacy = (current_category or "").strip().lower()
    ...
    if canonical:
        ...
        if category:
            return ClassificationResult(category.id, category.code, 0.75, None, f"Применено legacy-сопоставление {legacy} → {canonical}.", "review")

    heuristic = _heuristic_category(db, title=title, description=description)
    if heuristic:
        return ClassificationResult(heuristic.id, heuristic.code, 0.55, None, "Категория предложена по названию или описанию.", "review", ["Требуется ручная проверка."])
...
def persist_decision(db: Session, *, place_id: int, result: ClassificationResult, actor: str, old_category_id: int | None, batch_id: str | None = None) -> TaxonomyDecision:
    decision = TaxonomyDecision(
        ...
        mapping_id=result.matched_rule_id,
...
def _mapping_candidates(db: Session, *, source: str | None, tags: dict[str, str]) -> list[tuple[float, TaxonomyMapping]]:
    ...
    if source:
        query = query.filter(TaxonomyMapping.source.in_([source, "any", "text_alias"]))
    candidates: list[tuple[float, TaxonomyMapping]] = []
    for mapping in query.order_by(TaxonomyMapping.priority.desc()).all():
        if not _mapping_matches(mapping, tags):
            continue
        conditions = mapping.conditions or {}
        if any(tags.get(str(key).lower()) != str(value).lower() for key, value in conditions.items()):
            continue
...
def _mapping_matches(mapping: TaxonomyMapping, tags: dict[str, str]) -> bool:
    if mapping.source == "text_alias":
        text = f"{tags.get('__title__', '')} {tags.get('__description__', '')}".lower()
        return mapping.source_value.lower() in text
...
def _heuristic_category(db: Session, *, title: str | None, description: str | None) -> Category | None:
    ...
    aliases = {
        "pharmacy": ("аптек", "pharmacy"), "shopping_mall": ("торговый центр", "трц", "mall"),
        "bank": ("банк",), "atm": ("банкомат", "atm"), "museum": ("музей", "museum"),
        "park": ("парк",), "coffee": ("кофейн", "coffee"), "hospital": ("больниц", "hospital"),
    }
...
def _decision(confidence: float, *, ambiguous: bool) -> str:
    if confidence >= HIGH_CONFIDENCE and not ambiguous:
        return "auto_apply"
