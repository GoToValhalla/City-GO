...
def normalize_city_categories(db:Session,*,city_slug:str,apply:bool=True,job_id:int|None=None)->dict[str,object]:
 city=db.query(City).filter(City.slug==city_slug).first()
 if city is None:raise ValueError(f"Город не найден: {city_slug}")
 result=normalize_places_categories(db,places=db.query(Place).filter(Place.city_id==city.id).all(),apply=apply,job_id=job_id)
 if apply:db.commit()
 return result


def normalize_places_categories(db:Session,*,places:list[Place],apply:bool=True,job_id:int|None=None)->dict[str,object]:
 scanned=updated=synced=skipped=unknown=conflicts=0
 category_cache:dict[str,Category|None]={}
 for place in places:
  scanned+=1
  if apply:
   automated=normalize_place(db,place,actor="import_pipeline",job_id=job_id)
  ...
  if apply:
   proposed={"category":canon,"canonical_category":canon}
   if category_id:proposed["category_id"]=category_id
   if not propose_place_change(db,place=place,proposed=proposed,reason="category_normalization",job_id=job_id):
