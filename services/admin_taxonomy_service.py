...
def admin_category_taxonomy(db: Session) -> list[dict[str, object]]:
    catalog = {row.code: row for row in db.query(Category).order_by(Category.code.asc()).all()}
    observed = _observed_counts(db)
...
def _observed_counts(db: Session) -> dict[str, int]:
    rows = (
        db.query(func.lower(Place.category).label("code"), func.count(Place.id).label("count"))
        .filter(Place.category.is_not(None), func.trim(Place.category) != "")
        .group_by(func.lower(Place.category))
        .all()
    )
