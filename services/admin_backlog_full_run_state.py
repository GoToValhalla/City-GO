...
def create_full_run(db: Session, *, actor: str) -> dict[str, Any]:
    ...
    operation = AdminOperation(
        ...
        result={
            ...
            "actions": [
                {"action_code": code, "title": ACTION_TITLES[code], "status": "pending", "affected_count": 0, "changed_count": 0, "queued_count": 0, "skipped_count": 0, "failed_count": 0, "started_at": None, "finished_at": None, "message": None}
...
def _recompute(result: dict[str, Any], now: datetime) -> None:
    actions = list(result.get("actions") or [])
    done = {"applied", "completed", "partial", "failed", "unsupported"}
