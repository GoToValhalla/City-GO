...
HOURS_REQUIRED_CATEGORIES = MUSEUM_CATEGORIES | RESTAURANT_CATEGORIES
...
def quality_summary(
    ...
    for city in cities:
        ...
        if agg_row is None:
            ...
            quality = _build_city_quality_row_from_agg(
                city,
                agg_row,
...
def _category_key_expr():
    return func.lower(func.coalesce(Place.canonical_category, Place.category, ""))


def _tourist_review_condition():
    category = _category_key_expr()
...
def _critical_conditions() -> dict[str, Any]:
    ...
    card_blocker = missing_photo | missing_description | missing_address_required
    ...
    return {
        ...
        "missing_opening_hours_applicable": missing_opening_hours_applicable,
        ...
        "has_opening_hours_applicable": has_opening_hours_applicable,
    }
...
def _build_city_quality_row_from_agg(city: City, agg_row: Any, *, pending_photo_review_total: int = 0) -> dict[str, Any]:
    ...
    manual_review_total = int(pending_photo_review_total or 0)
    ...
    blockers = {
        ...
        for key, value in {
            ...
            "no_description": no_description_total,
            "missing_opening_hours": missing_opening_hours_total,
        }.items()
        if value
    }
    readiness = _readiness_score(review_universe_total, route_blockers_total, card_blockers_total)
    ...
    critical_coverage = _fast_critical_coverage(
        ...
        pending_photo_review_total=manual_review_total,
    )
    return {
...
def _readiness_score(review_universe_total: int, route_blockers_total: int, card_blockers_total: int) -> int:
    if review_universe_total <= 0:
        return 0
    penalty = 50 * (route_blockers_total / review_universe_total) + 25 * (card_blockers_total / review_universe_total)
...
def _primary_blocker(blockers: dict[str, int], readiness: int) -> str | None:
    ...
    if blockers.get("no_address"):
        return "no_address"
    if blockers.get("no_description"):
...
def _fast_critical_coverage(
    *,
    ...
    route_ready_total: int,
    route_blockers_total: int,
    card_ready_total: int,
    card_blockers_total: int,
    manual_review_total: int,
    excluded_total: int,
    no_photo_total: int,
    no_address_total: int,
    no_description_total: int,
    missing_required_address_total: int,
    missing_opening_hours_total: int,
    missing_opening_hours_applicable_total: int,
    route_ineligible_total: int,
    missing_category_total: int,
    unknown_category_total: int,
    hours_applicable_total: int,
    has_opening_hours_total: int,
    pending_photo_review_total: int,
) -> dict[str, Any]:
    """Return smoke-safe critical coverage counters from aggregate SQL results.

    The full triage engine loads every place and several related tables. This summary
    keeps the default admin quality read bounded while preserving the public contract
    used by critical coverage dashboards and smoke tests.
    """
    coverage = {
        "has_approved_photo": {
            "count": max(0, review_universe_total - no_photo_total),
            "total": review_universe_total,
            "pct": _pct(max(0, review_universe_total - no_photo_total), review_universe_total),
        },
        "has_address": {
            "count": max(0, review_universe_total - no_address_total),
            "total": review_universe_total,
            "pct": _pct(max(0, review_universe_total - no_address_total), review_universe_total),
        },
        "has_description": {
            "count": max(0, review_universe_total - no_description_total),
            "total": review_universe_total,
            "pct": _pct(max(0, review_universe_total - no_description_total), review_universe_total),
        },
        "has_opening_hours": {
            "count": has_opening_hours_total,
            "total": hours_applicable_total,
            "pct": _pct(has_opening_hours_total, hours_applicable_total),
        },
    }
    route_ready_pct = _pct(route_ready_total, review_universe_total)
    route_blockers_breakdown = {
        key: value
        for key, value in {
            "missing_opening_hours": missing_opening_hours_total,
            "route_ineligible": route_ineligible_total,
            "missing_canonical_category": missing_category_total,
            "unknown_category": unknown_category_total,
        }.items()
        if value
    }
    card_blockers_breakdown = {
        key: value
        for key, value in {
            "missing_image_url": no_photo_total,
            "missing_short_description": no_description_total,
            "missing_address": missing_required_address_total,
        }.items()
        if value
    }
    return {
        ...
        "tourist_places": {
            "total": review_universe_total,
            "route_ready": route_ready_total,
            "route_blocked": route_blockers_total,
            "card_ready": card_ready_total,
            "card_blocked": card_blockers_total,
            "excluded_non_tourist": excluded_total,
        },
        ...
        "manual_review_total": manual_review_total,
        "optional_gaps_total": max(0, missing_opening_hours_applicable_total - missing_opening_hours_total),
        "not_applicable_total": excluded_total,
        "route_blockers_breakdown": route_blockers_breakdown,
        "card_blockers_breakdown": card_blockers_breakdown,
        ...
        "manual_review_queue": {
            "total": manual_review_total,
            "pending_photo_review": pending_photo_review_total,
            "source_conflicts": 0,
        ...
        "city_readiness": {
            ...
            "is_launch_ready": route_ready_pct >= 70.0 and route_blockers_total == 0,
            "blocking_reason": None if route_ready_pct >= 70.0 and route_blockers_total == 0 else "route_ready_pct below threshold or route blockers exist",
        },
    }
...
def _todo(rows: list[dict[str, Any]]) -> list[str]:
    ...
    if not critical:
        return []
    return [
        f"{row['city_name']}: проверить {row['manual_review_total']} мест"
