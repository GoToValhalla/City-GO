...
def run_enrichment_pipeline(db:Session,*,job:CityAdminImportJob,city:City,actor_id:str,force:bool=True,notify_completion:bool=True)->dict[str,Any]:
 original=(city.launch_status,bool(city.is_active)); started=datetime.utcnow(); warnings=[]; results={}
 ...
 try:
  ...
  total=db.query(Place).filter(Place.city_id==city.id).count();set_step(job,STEP_COLLECTING_PLACES,total=total,processed=total,successful=job.places_saved,detail={"import_diff":summary});db.commit()
  if summary.get("status")!="success" and total<=0:raise RuntimeError(str(summary.get("last_error") or "Ошибка импорта OSM"))
  if summary.get("status")!="success":
   warning={"step":STEP_COLLECTING_PLACES,"error":str(summary.get("last_error") or "partial import")};warnings.append(warning);append_step_warning(job,STEP_COLLECTING_PLACES,warning["error"])
  addresses=_optional(db,job,city.slug,actor_id,STEP_FINDING_ADDRESSES,warnings,lambda:run_address_backfill(["--city",city.slug,"--limit",str(ADDRESS_LIMIT),"--apply"]));results["addresses"]=addresses
  set_step(job,STEP_FINDING_ADDRESSES,processed=int(addresses.get("checked") or 0),successful=int(addresses.get("updated") or 0),failed=int(addresses.get("errors") or 0));db.commit()
  images=_optional(db,job,city.slug,actor_id,STEP_FINDING_IMAGES,warnings,lambda:run_image_enrich(["--city",city.slug,"--limit",str(IMAGE_LIMIT),"--apply"]));results["images"]=images
  set_step(job,STEP_FINDING_IMAGES,processed=int(images.get("scanned_places") or 0),successful=int(images.get("created") or 0),failed=int(images.get("failed_image_lookup") or images.get("failed") or 0));db.commit()
  db.expire_all();places=_changed(db,city.id,started)
  for place in places:mark_place_for_review(place,reason="import_or_enrichment_changed")
  db.commit();set_step(job,STEP_PREPARING_DESCRIPTIONS,detail={"mode":"manual_required"})
  cats=normalize_places_categories(db,places=places,apply=True);results["categories"]=cats;set_step(job,STEP_CATEGORIES_TAGS,processed=int(cats.get("scanned") or 0),successful=int(cats.get("updated") or 0));db.commit()
  ids=sorted({int(p.id) for p in places});results.update(changed_place_ids=ids,has_changes=bool(ids),quality={"mode":"foundation","changed_places":len(ids)})
  set_step(job,STEP_COMPUTING_QUALITY,processed=len(ids),detail=results["quality"]);set_step(job,STEP_COMPUTING_READINESS)
  ...
  set_step(job,STEP_READY_FOR_REVIEW,successful=len(ids),processed=len(ids));job.status="success_with_warnings" if warnings else "success";job.finished_at=datetime.utcnow()
  job.step_details={**dict(job.step_details or {}),"warnings":warnings,"changed_place_ids":ids,"has_changes":bool(ids),"import_summary":summary}
  ...
  db.rollback();db.expire_all();places=_changed(db,city.id,started);ids=sorted({int(p.id) for p in places})
  for place in places:mark_place_for_review(place,reason="partial_import_changed")
  total=db.query(Place).filter(Place.city_id==city.id).count();job.last_error=str(exc)[:2000];job.finished_at=datetime.utcnow();job.step_details={**dict(job.step_details or {}),"changed_place_ids":ids,"has_changes":bool(ids)}
  if total>0:
...
def _notify(city,job,total,changed,readiness,warnings):
...
def _optional(db,job,slug,actor,step,warnings,action):
 ...
 try:
  result=action();_log(db,job,slug,actor,step,"success");return result if isinstance(result,dict) else {"result":result}
 except Exception as exc:
  warnings.append({"step":step,"error":str(exc)[:1000]});append_step_warning(job,step,exc);_log(db,job,slug,actor,step,"warning",error=str(exc));db.commit();return {"status":"warning","error":str(exc)[:1000]}
