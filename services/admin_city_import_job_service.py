...
def _queue_job(db: Session, *, city: City, source: str, actor_id: str | None) -> CityAdminImportJob:
    ...
    job.failed_items = 0
    job.step_details = {
        "city_state_before_import": {"launch_status": city.launch_status, "is_active": bool(city.is_active)},
        **preserved,
    }
    job.started_at = None
...
def run_address_enrichment_job(db: Session, *, city_id: int, actor_id: str) -> CityAdminImportJob:
    ...
    job.step_details = {**dict(job.step_details or {}), "address_enrichment": result}
    job.status = "success"
    job.finished_at = datetime.utcnow()
    job.current_step = "snapshot_refresh"
    db.commit()
...
def run_enrichment_only_job(db: Session, *, city_id: int, actor_id: str) -> CityAdminImportJob:
    ...
    try:
        ...
        db.refresh(job)
        _foundation(db, city, job, actor_id, [int(v) for v in ((job.step_details or {}).get("changed_place_ids") or [])])
