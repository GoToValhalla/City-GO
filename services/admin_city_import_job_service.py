...
def run_city_import_job(db: Session, *, city_id: int, actor_id: str) -> CityAdminImportJob:
    ...
    try:
        ...
        city.last_import_at = job.finished_at; refresh_import_job_snapshot(db, city_id=city.id, source="import_worker_finished")
...
def run_snapshot_refresh_job(db: Session, *, city_id: int, actor_id: str) -> CityAdminImportJob:
    ...
    refresh_import_job_snapshot(db, city_id=city_id, source="snapshot_refresh_job")
...
def run_address_enrichment_job(db: Session, *, city_id: int, actor_id: str) -> CityAdminImportJob:
    ...
    job.step_details = {**dict(job.step_details or {}), "address_enrichment": result}; job.status = "success"; job.finished_at = datetime.utcnow(); job.current_step = "snapshot_refresh"; db.commit(); refresh_import_job_snapshot(db, city_id=city_id, source="address_enrichment_finished"); db.refresh(job); return job
...
def run_photo_enrichment_job(db: Session, *, city_id: int, actor_id: str) -> CityAdminImportJob:
    ...
    job.step_details = {**dict(job.step_details or {}), "photo_enrichment": result}; job.status = "success"; job.finished_at = datetime.utcnow(); job.current_step = "snapshot_refresh"; db.commit(); refresh_import_job_snapshot(db, city_id=city_id, source="photo_enrichment_finished"); db.refresh(job); return job
...
def run_enrichment_only_job(db: Session, *, city_id: int, actor_id: str) -> CityAdminImportJob:
    ...
    try: run_enrichment_only_pipeline(db, job=job, city=city, actor_id=actor_id); db.refresh(job); _foundation(db, city, job, actor_id, [int(v) for v in ((job.step_details or {}).get("changed_place_ids") or [])]); refresh_import_job_snapshot(db, city_id=city_id, source="enrichment_only_finished")
