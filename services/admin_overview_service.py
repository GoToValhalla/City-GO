...
def build_admin_overview(db: Session) -> dict[str, object]:
    ...
    data_quality = [
        _card("route_blockers", "Не попадут в маршруты", policy_counts["blocked"], _sev(policy_counts["blocked"], 10, 100), "/admin/quality?critical_bucket=route_blocker", "Published/catalog places, которые не проходят route policy.", "Открыть блокеры"),
        _card("route_unknown", "Маршруты: неизвестная категория", policy_counts["unknown"], _sev(policy_counts["unknown"], 10, 100), "/admin/quality?critical_bucket=route_blocker&critical_reason=unknown_category", "Нужно исправить canonical category или пересчитать taxonomy.", "Открыть unknown"),
        _card("route_excluded", "Сервисные точки опубликованы", policy_counts["excluded"], _sev(policy_counts["excluded"], 1, 20), "/admin/quality?critical_bucket=route_blocker", "Сервисные точки в published/catalog слое.", "Открыть сервисные"),
        _card("auto_backlog", "Автоочередь enrichment/policy", place_counts["auto_backlog"], _sev(place_counts["auto_backlog"], 500, 5000), "/admin/quality?critical_bucket=auto_enrichment_candidate", "Не ручная модерация: запускать автообогащение и пересчёт policy.", "Открыть авто"),
        _card("manual_review", "Ручная проверка", place_counts["manual_review"], _sev(place_counts["manual_review"], 20, 100), "/admin/quality?critical_bucket=manual_review", "Только то, что автоматом нельзя решить.", "Открыть ручную очередь"),
        _card("needs_verification", "Автоперепроверка", place_counts["needs_verification"], _sev(place_counts["needs_verification"], 100, 1000), "/admin/verification?queue=verification", "Verification backlog, не ручная очередь.", "Открыть verification"),
        _card("no_photo", "Опубликовано без фото", place_counts["no_photo"], _sev(place_counts["no_photo"], 20, 100), "/admin/quality?critical_bucket=card_blocker&critical_reason=missing_image_url", action_label="Открыть фото debt"),
        _card("no_address", "Без адреса", place_counts["no_address"], _sev(place_counts["no_address"], 30, 150), "/admin/quality?critical_bucket=card_blocker&critical_reason=missing_address", action_label="Открыть адреса"),
        _card("no_description", "Без описания", place_counts["no_description"], _sev(place_counts["no_description"], 40, 200), "/admin/quality?critical_bucket=card_blocker&critical_reason=missing_short_description", "Пустое или слишком короткое описание.", "Открыть описания"),
        _card("low_confidence", "Низкая уверенность", place_counts["low_confidence"], _sev(place_counts["low_confidence"], 15, 80), "/admin/quality?critical_bucket=manual_review&critical_reason=low_confidence_critical_field", "Critical confidence bucket, не общая ручная очередь.", "Открыть confidence"),
    ]
