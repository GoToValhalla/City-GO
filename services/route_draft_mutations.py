...
from services.route_draft_reconcile import reconcile_draft_public_points
...
def remove_point(db: Session, draft: RouteDraft, point_id: int, version: int) -> RouteDraft:
    ...
    with db.begin_nested():
        reconcile_draft_public_points(db, draft)
    ...
    with db.begin_nested():
        ...
        db.add(
            RouteDraftPoint(
                ...
                visit_minutes=visit_minutes_for(place),
            )
        )
        db.flush()
        ...
        if draft.total_minutes > draft.budget_minutes:
            draft.warnings = _append_warning(
                draft, "OVER_BUDGET", "Маршрут вышел за выбранный лимит времени."
            )
    return _commit(db, draft)


def replace_point(
    db: Session, draft: RouteDraft, point_id: int, replacement_place_id: int, version: int
) -> RouteDraft:
    check_version(draft, version)
    with db.begin_nested():
        reconcile_draft_public_points(db, draft)
...
def _commit(db: Session, draft: RouteDraft) -> RouteDraft:
    db.commit()
    db.refresh(draft)
    return draft
