...
def evaluate_place_route_eligibility(place: Place, *, city: City | None = None, context: str = "tourist_walk") -> RouteEligibilityResult:
    ...
    policy = evaluate_category_policy(getattr(place, "category_ref", None), context=context)
    if not policy.allowed:
        reasons.append("route_policy_review" if policy.requires_review else f"route_policy_denied:{context}")

    quality_tier = (getattr(place, "quality_tier", None) or "silver").strip().lower()
    if quality_tier not in ROUTE_ALLOWED_QUALITY_TIERS: reasons.append(f"quality_tier_not_route_allowed:{quality_tier or 'empty'}")
