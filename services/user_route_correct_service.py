...
from schemas.user_route import UserRouteCorrectRequest, UserRouteIntent
...
class UserRouteCorrectService:
    ...
    def _intent_for_request(
        ...
        avoided = [
            target_category
        ] if request.action == "avoid_category" and isinstance(target_category, str) and target_category else []
        target_exclusion = [
            request.target_place_id
        ] if request.action == "remove_place" and target is not None else []
        return with_updates(
            intent,
            lat=request.current_lat,
            lng=request.current_lng,
            time_budget_minutes=request.new_time_budget_minutes,
            avoided_categories=merge_unique(
                intent.avoided_categories,
                [*request.avoided_categories, *avoided],
            ),
