"""Idempotent, ownership-tracked column/index/constraint helpers for Alembic
migrations that add columns to a table which may already have them in
...
form). The fix is not just "skip if it exists" — a later `alembic
downgrade` must also not drop a column that predates this revision, so
each column/index/constraint this module creates is tagged with a SQL
COMMENT recording which revision created it. downgrade() only drops
objects whose comment marks them as owned by that revision; a compatible,
pre-existing object (no comment, or a comment from a different revision)
is left untouched.
