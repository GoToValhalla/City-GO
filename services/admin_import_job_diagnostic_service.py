...
def _failed_steps(steps: list[ImportJobStep]) -> list[dict[str, object]]:
    return [
        {
...
def _partial_success_reason(job: CityAdminImportJob, failed_steps: list[dict[str, object]]) -> str | None:
    ...
    never set on this path — only on a hard failure — so the actual reason
    lives in the per-step ImportJobStep.error_message rows instead. Without
    this, an admin has no field explaining why a partial_success run was
    partial short of reading raw step_details JSON."""
    if job.status != "partial_success" or not failed_steps:
        return None
    parts = [
        f"{step['step_label']}: {step['error_message']}" if step.get("error_message") else str(step["step_label"])
...
def _build_report_text(
    *,
    ...
    if partial_success_reason:
        lines.append(f"Partial success reason: {partial_success_reason}")
