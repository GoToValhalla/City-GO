...
_SKELETON_PRESERVED_WARNING = "route_skeleton_preserved_after_edit"
...
class UserRouteRecalcService:
    def recalc(
        ...
        points = _to_points(places)
        route = TimeAwareService().apply(points, ctx, datetime.utcnow())
        warnings = [*_default_warning(places), *(extra_warnings or [])]
        if route:
