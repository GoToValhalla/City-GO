...
def _failure_details(text: str, message: str) -> tuple[str | None, str | None, str | None]:
    ...
    rules = [
        ("MultipleResultsFound", "Нарушена уникальность: код ожидал одну запись, но получил несколько.",
         "Проверить идемпотентность создания записей и уникальный ключ выборки."),
        ("NoResultFound", "Ожидаемая запись не была создана или была отфильтрована.",
         "Проверить setup теста и условие выборки перед вызовом one()."),
        ("has no attribute", "Тест и реализация рассинхронизированы: тест обращается к удалённому или переименованному API.",
         "Обновить monkeypatch/import на фактически вызываемую функцию и закрепить контракт тестом."),
        ("Unable to find an element", "Интерфейс не отрендерил ожидаемый элемент либо компонент упал раньше проверки.",
         "Проверить stderr выше, обязательные Provider/Router и актуальность пользовательского текста."),
        ("useContext", "React-компонент запущен без обязательного контекстного Provider.",
         "Обернуть тест в Router/Provider, который используется компонентом в приложении."),
        ("IntegrityError", "База отклонила запись из-за ограничения целостности.",
         "Проверить уникальные ключи, внешние ключи и очистку данных между тестами."),
        ("AssertionError", "Фактическое поведение не совпало с ожидаемым контрактом теста.",
         "Сравнить expected/actual в полном логе и определить: регрессия продукта или устаревшее ожидание."),
    ]
...
def parse_junit(path: Path) -> TestSummary:
    ...
    for case in cases:
        ...
        summary.failed_tests.append(FailedTest(
            classname=classname,
...
def render_message(summary: TestSummary, args: argparse.Namespace) -> str:
    ...
    if summary.failed_tests:
        ...
        for index, failed in enumerate(summary.failed_tests[:8], start=1):
